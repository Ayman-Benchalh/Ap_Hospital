...
$errClinic = $errFName = $errLName = $errSpec = $errYears = $errFee = $errSpoke = $errGender = $errEmail = $errContact = $errImage = $errPassword=$errConfPassword= "";


$classClinic = $classFName = $classLName = $classSpec = $classYears = $classFee = $classSpoke = $classGender = $classEmail = $errContact = $classPassword = $classConfPassword="";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    elseif ($password !== $confPassword) {
        // echo "password confirme not good";
        $errConfPassword = $error_html['passwordsDoNotMatch'];
        $classConfPassword = $error_html['errClass'];
    }elseif($password === $confPassword){
            // echo "password confirme  good";
    }
    // else{
    //     echo "password confirme good";
    // }
                ...
                <form name="regform" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" enctype="multipart/form-data">
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    <label for="inputPassword">Add Password</label>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" name="inputConfPassword" class="form-control <?= $classConfPassword ?>" id="inputConfPassword" placeholder="Enter Confirme Password">
                                    <?= $errConfPassword ?>
                                </div>
                            </div>
                        </div>
...
if (isset($_POST["savebtn"])) {
    ...
    if (multi_empty($errFName, $errLName, $errSpec, $errYears, $errFee, $errSpoke, $errGender, $errEmail, $errContact, $errImage,$errPassword ,$errConfPassword)) {
        ...
        echo "data Doctor : " . $fname, $lname, $speciality, $years, $desc, $spoke, $gender, $dob, $email, $contact, $fees, $date_created, $clinic_id;
        echo "<br> data Doctor password: " .$password ," confi  ", $confPassword ,"hashing pass " . $cryp_Password ;
        ...
        $stmt->bind_param("ssssisssssssssi", $image, $fname, $lname, $speciality, $years, $desc,$cryp_Password , $spoke, $gender, $dob, $email, $contact, $fees, $date_created, $clinic_id);
        if ($stmt->execute()) {
